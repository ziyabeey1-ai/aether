...
Consider best practices for ${profile.siteType} websites and the target audience.
  ...
  // Using gemini-3-pro-preview as it is the stable model for complex reasoning tasks
  const response = await ai.models.generateContent({
    model: 'gemini-3-pro-preview',
    contents: prompt,
    config: {
      thinkingConfig: { thinkingBudget: 4096 }, // Adjusted budget for pro model
...
/**
 * Generate logo using AI if user doesn't have one
 */
